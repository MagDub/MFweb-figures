function [] = render_score_perblock_LH_1st(to_del)
    ...
    % plot
    n_blocks = 4;
    [beta_first_LH, stats_slope] = fct_compute_coeff(first_LH, n_blocks);
    
    disp('Block - 1st LH')
    disp(stats_slope)
    disp('--------------')
    
    fct_plot_score(n_blocks, nanmean(first_LH,1), beta_first_LH, {'Long horizon','1st sample'}, 'b', 'NorthWest', first_LH)
