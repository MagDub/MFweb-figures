...
for id = 1:size(first_LH,1)
    
    t_score_SH(id,:) = X\score_SH(id,:)';
...
% 
% % mean b0 and b1
% b0 = nanmean(t_score_SH(:,1));
% b1 = nanmean(t_score_SH(:,2));
% 
% % plot linear reg
% plot(x, b0+x*b1)
%  
...
%%%% Stats
reward_slopes = t_score_SH(:,2);
null_slopes = zeros(size(t_score_SH,1),1);
