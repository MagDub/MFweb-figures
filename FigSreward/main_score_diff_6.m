...
% plot bars
ph = plot_score_bars(score_H(:,2)-score_H(:,3), score_H(:,5)-score_H(:,6), score_H(:,14)-score_H(:,6), [3,7,11], x_ax, 0.1);
pc = plot_score_bars(score_C(:,2)-score_C(:,3), score_C(:,5)-score_C(:,6), score_C(:,14)-score_C(:,6), [4,8,12], x_ax, 0.4);
pd = plot_score_bars(score_D(:,2)-score_D(:,3), score_D(:,5)-score_D(:,6), score_D(:,14)-score_D(:,6), [5,9,13], x_ax, 0.8);
...
max_y = 3; %max([max(score_H(:,2:end)), max(score_C(:,2:end)), max(score_D(:,2:end))]);
min_y = -4;
...
% Export
addpath('../../export_fig')
export_fig(['Fig_score_perB_diff_mean_6.tif'],'-nocrop','-r200')
