...
function [] = render_param_recovery()
    ...
    h = colorbar('Ticks',0:0.2:1, ...
        'Position', [0.913580246913578 0.109311740890688 0.0167989417989416 0.313765182186235]); 
    caxis([0 1])
    ...
    %xlh.Position(2) = xlh.Position(2) + 2.38;

    hStrings = text(x(:), y(:), textStrings(:),'HorizontalAlignment', 'center','Fontweight','normal','FontSize',11, 'Color', 'k');
    midValue = mean(get(gca, 'CLim'));  % Get the middle value of the color range
    
    % Number and title
    text(0-0.15, 1+0.2,'h','Units', 'Normalized', 'VerticalAlignment', 'Top','FontSize', 26)
