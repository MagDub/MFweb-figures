...
function [] = plot_time_task(col_)
    ...
    sum_dur_4blocks_s(sum_dur_4blocks_s<0)=nan; % some are neg, check, maybe change of timezone?
    ...
    % mean
    bar_plot = bar(1,nanmean(sum_dur_4blocks_m), 'FaceAlpha', 0.2, 'BarWidth',.5, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',0.5); hold on;
    
    disp(nanmean(sum_dur_4blocks_m))
    disp(nanstd(sum_dur_4blocks_m))

    % individual dots
    h = scatter(ones(1,n)+noise_plot, sum_dur_4blocks_m, 16, col_(2,:), 'filled', 'MarkerFaceAlpha', 0.1); hold on;
