...
function [] = plot_score_exclusion(col_)
    ...
    % mean
    bar_plot = bar(1,nanmean(score), 'FaceAlpha', 0.2, 'BarWidth',.5, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',0.5); hold on;

    % individual dots
    h = scatter(ones(1,n)+noise_plot, score, 18, col_(2,:), 'filled', 'MarkerFaceAlpha', 0.1); hold on;
