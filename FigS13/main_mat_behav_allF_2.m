function [] = main_mat_behav_allF_2(c_ax, inc)  

    addpath('../cmocean/')

    path = '../../data_analysis/10_stats/corr_bonferroni/corr_allQ/';
    file_r = 'allF_behav_r.csv';
    file_pc = 'allF_behav_pval_corr.csv';
    file_pu = 'allF_behav_pval_uncorr.csv';
    ...
    xticks([1:3])
    xlim([0.5 3.5])
    xticklabels({'  Anxious-\newlineDepression', 'Uncertainty-\newline   related',  'Impulsivity'})
    xtickangle(0)
    ...
    yticklabels({...
        ...
        strcat(32,32,'Novel \newline bandit'), ...
        strcat('High-value \newline',32,32,32,32,32,32,32,'bandit')}) 
    ...
    text(0-0.4, 1+0.25,'d','Units', 'Normalized', 'VerticalAlignment', 'Top','FontSize', 26)  
end
