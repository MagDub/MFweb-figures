function [p] = render_corr(var1, var2, param_bounds_var1, param_bounds_var2, title_, letter, legend_position, var1_label, var2_label, corr_string_x, corr_string_y)
...
% string output
if p<0.001
    corr_string = {strcat('r=',num2str(round(r,2)));strcat('p<.001')}; 
else
    corr_string = {strcat('r=',num2str(round(r,2)));strcat('p=',num2str(round(p,3)))}; 
end
...
% Number and title
text(corr_string_x, corr_string_y, corr_string,'Units', 'Normalized', 'VerticalAlignment', 'Top','FontSize', 16, 'Fontweight','normal')
text(0-0.3, 1+0.25,letter,'Units', 'Normalized', 'VerticalAlignment', 'Top','FontSize', 26)  
title(title_,'FontSize', 17, 'FontName','Arial', 'Fontweight','normal');

l = legend(b1,''); 
set(l,'Location',legend_position, 'FontSize',2);
legend boxoff;
