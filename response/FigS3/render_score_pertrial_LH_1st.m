function [] = render_score_pertrial_LH_1st(min_, max_, indexation_y, indexation_x, n_trials)
    ...
    for i_ = 1:n_trials:400
        first_LH_av(:,end+1) = sum(first_LH(:,i_:(i_+(n_trials-1))),2)/n_trials;
    end
    ...
    % plot
    plot([n_trials:n_trials:400],nanmean(first_LH_av,1)-nanstd(first_LH_av)./sqrt(n),'LineWidth',1, 'Color',[col(2,:) 0.3]); hold on;
    plot([n_trials:n_trials:400],nanmean(first_LH_av,1)+nanstd(first_LH_av)./sqrt(n),'LineWidth',1, 'Color',[col(2,:) 0.3]); hold on;
    plot([n_trials:n_trials:400],nanmean(first_LH_av,1),'LineWidth',2,'Color',[col(2,:) 1]); hold on;
    ...
    t=title({'Long horizon','1st sample'},'FontSize', 18, 'FontName','Arial', 'Fontweight','normal');
