...
function [] = render_etaT(ybounds, increment, signif, hight_signif)

    
    %% Data
    load('../../../data_analysis/usermat_completed.mat')
    
    load('../../../data/data_for_figs/model_parameters_mod12_etaT.mat')
    load('../../../data/data_for_figs/model_parameters_mod12_etaT_desc.mat')
    
    ind_SH = find(contains(model_parameters_desc,'eta_short'));
    ind_LH = find(contains(model_parameters_desc,'eta_long'));
    ind_SH_T = find(contains(model_parameters_desc,'etaT_short'));
    ind_LH_T = find(contains(model_parameters_desc,'etaT_long'));
    ...
    title(['\fontsize{18}Trial-dependent {\fontsize{22}\eta}'],'interpreter','tex','Fontweight','normal','FontName','Arial');

    xlim([1 5])   
    set(gca,'XTick',[2.25 3.75])
    set(gca,'XTickLabel',{'\fontsize{15} \eta','\fontsize{15} \eta_T'})
    set(gca,'box','off')
