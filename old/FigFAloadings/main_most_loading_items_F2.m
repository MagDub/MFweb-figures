...
[data_load_F2, I] = sort(T.loading_F2, 'descend');
data_item = T.Item(I);

T2 = table(data_item, data_load_F2);
...
subplot(2,1,1)
hold all;
bar(item_no(BIS_ind), T2.data_load_F2(BIS_ind), 'FaceColor', col_(contains(col_desc,'BIS'),:));
bar(item_no(ASRS_ind), T2.data_load_F2(ASRS_ind), 'FaceColor', col_(contains(col_desc,'ASRS'),:));
bar(item_no(AQ10_ind), T2.data_load_F2(AQ10_ind), 'FaceColor', col_(contains(col_desc,'AQ10'),:));
bar(item_no(CFS_ind), T2.data_load_F2(CFS_ind), 'FaceColor', col_(contains(col_desc,'CFS'),:));
bar(item_no(OCIR_ind), T2.data_load_F2(OCIR_ind), 'FaceColor', col_(contains(col_desc,'OCIR'),:));
bar(item_no(SDS_ind), T2.data_load_F2(SDS_ind), 'FaceColor', col_(contains(col_desc,'SDS'),:));
bar(item_no(STAI_ind), T2.data_load_F2(STAI_ind), 'FaceColor', col_(contains(col_desc,'STAI'),:));
bar(item_no(IUS_ind), T2.data_load_F2(IUS_ind), 'FaceColor', col_(contains(col_desc,'IUS'),:));
bar(item_no(LSAS_ind), T2.data_load_F2(LSAS_ind), 'FaceColor', col_(contains(col_desc,'LSAS'),:));
hold off;
...
legend boxoff;
title('Factor 2 item loads','FontSize', 18, 'FontName','Arial', 'Fontweight','normal');
set(gca,'box','off')
...
[data_load_F2, I] = sort(abs(T.loading_F2), 'descend');
data_item = T.Item(I);
Tabs = table(data_item, data_load_F2);
...
%Export
addpath('../../../export_fig')
export_fig(['Fig_items_F2.tif'],'-nocrop','-r200')