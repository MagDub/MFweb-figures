function [] = render_score_pertrial_LH_all(min_, max_, indexation_y, indexation_x, n_trials)
    ...
    for i_ = 1:n_trials:400
        score_LH_av(:,end+1) = sum(score_LH(:,i_:(i_+(n_trials-1))),2)/n_trials;
    end
    ...
    % plot
    plot([n_trials:n_trials:400],nanmean(score_LH_av,1)-nanstd(score_LH_av)./sqrt(n),'LineWidth',1, 'Color',[col(2,:) 0.3]); hold on;
    plot([n_trials:n_trials:400],nanmean(score_LH_av,1)+nanstd(score_LH_av)./sqrt(n),'LineWidth',1, 'Color',[col(2,:) 0.3]); hold on;
    plot([n_trials:n_trials:400],nanmean(score_LH_av,1),'LineWidth',2,'Color',[col(2,:) 1]); hold on;
