...
function [] = render_model_comparison_BIC_g1(col_, letter, subplot_title)
    ...
    % plot
    bar(x,mean_all(I), 'FaceAlpha', 0.2, 'BarWidth',.7, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',1.5); hold on;
    
    % best model
    [~, ind_best] = min(mean_all(I));
    bw = bar(x(ind_best),mean_all(I(ind_best)), 'FaceAlpha', 0.8, 'BarWidth',.7,'FaceColor',col_(2,:), 'EdgeColor', col_(2,:),'LineWidth',1.5); hold on;
    legend([bw],{'Winning model'}, 'Location','NorthEast');
    legend boxoff  

    er = errorbar(x,mean_all(I),stderror_all(I),stderror_all(I),'.','color',col_(2,:),'LineWidth',1.5);                            
    er.LineStyle = 'none';  
