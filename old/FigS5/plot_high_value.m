...
function [] = plot_high_value(col_, ybounds, increment, signif, hight_signif)
    ...
    % Short horizon
    b2S = bar(x_ax(3),nanmean(pickedhigh_SH), 'FaceAlpha', 0.2, 'BarWidth',1, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',1.2); hold on;

    % Long horizon
    b2L = bar(x_ax(6),nanmean(pickedhigh_LH), 'FaceAlpha', 0.2, 'BarWidth',1, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',1.2); hold on;
