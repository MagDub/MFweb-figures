...
function [] = render_epsilonB(ybounds, increment, signif, hight_signif)

    
    %% Data
    load('../../../data_analysis/usermat_completed.mat')
    
    load('../../../data/data_for_figs/model_parameters_mod12_xiB.mat')
    load('../../../data/data_for_figs/model_parameters_mod12_xiB_desc.mat')
    
    ind_SH = find(contains(model_parameters_desc,'xi_short'));
    ind_LH = find(contains(model_parameters_desc,'xi_long'));
    ind_SH_B = find(contains(model_parameters_desc,'xiB_short'));
    ind_LH_B = find(contains(model_parameters_desc,'xiB_long'));
    ...
    title(['\fontsize{18}Block-dependent {\fontsize{22}\epsilon}'],'interpreter','tex','Fontweight','normal','FontName','Arial');

    xlim([1 5])   
    set(gca,'XTick',[2.25 3.75])
    set(gca,'XTickLabel',{'\fontsize{15} \epsilon','\fontsize{15} \epsilon_B'})
    set(gca,'box','off')
