...
function [] = render_model_comparison_BIC_t1(col_, letter, subplot_title)

    load('../../../data/data_for_figs/mle_mat_all_t1.mat');
    ...
    legend_all{13} = 'UCB (\beta=1)';
    legend_all{14} = 'UCB (\beta=1) + \epsilon';
    legend_all{15} = 'UCB (\beta=1) + \eta';
    legend_all{16} = 'UCB (\beta=1) + \epsilon + \eta';
    ...
    er = errorbar(x,mean_all(I),stderror_all(I),stderror_all(I),'.','color',col_(2,:),'LineWidth',1.2);                            
    er.LineStyle = 'none';  
