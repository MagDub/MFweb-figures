...
function [] = plot_RT_exclusion(col_)
    ...
    % mean
    bar_plot = bar(1,nanmean(task_RT_1st_in_ms), 'FaceAlpha', 0.2, 'BarWidth',.5, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',1.2); hold on;

    % individual dots
    h = scatter(ones(1,n)+noise_plot, task_RT_1st_in_ms, 18, col_(2,:), 'filled', 'MarkerFaceAlpha', 0.3); hold on;
