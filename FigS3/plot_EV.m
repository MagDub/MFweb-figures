...
function [] = plot_EV(to_del, col_, ybounds, increment, signif, hight_signif)
    ...
    % SH
    bar(x_ax(3),nanmean(EV_SH_mat), 'FaceAlpha', 0.2, 'BarWidth',1, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',0.5); hold on;

    %b1S= bar(x_ax(3),nanmean(EV_SH_mat),'FaceColor',col_(1,:), 'FaceAlpha', 0.5, 'BarWidth',1); hold on;

    % first LH
    bar(x_ax(6),nanmean(EV_LH_mat), 'FaceAlpha', 0.2, 'BarWidth',1, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',0.5); hold on;

    %b1Lf = bar(x_ax(6),nanmean(EV_LH_mat),'FaceColor',col_(1,:), 'FaceAlpha', 0.5, 'BarWidth',1);

    for n = 1:size(EV_LH_mat,1)
        scatter(x_ax(3)+noise_plot(n), EV_SH_mat(n), 5, col_(2,:), 'filled', 'MarkerFaceAlpha', 0.09); hold on;
        scatter(x_ax(6)+noise_plot(n), EV_LH_mat(n), 5, col_(2,:), 'filled', 'MarkerFaceAlpha', 0.09); hold on;
