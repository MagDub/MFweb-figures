...
function [] = plot_score(to_del, col_, ybounds, increment, signif, hight_signif)
    ...
    % SH
    b1S= bar(x_ax(3),nanmean(score_SH), 'FaceAlpha', 0.2, 'BarWidth',1, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',0.5); hold on;

    % first LH
    b1Lf = bar(x_ax(6),nanmean(first_LH), 'FaceAlpha', 0.2, 'BarWidth',1, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',0.5); hold on;

    % LH
    b1L = bar(x_ax(9),nanmean(score_LH), 'FaceAlpha', 0.2, 'BarWidth',1, 'FaceColor',col_(1,:), 'EdgeColor', col_(2,:),'LineWidth',0.5); hold on;

    for n = 1:size(first_LH,1)
        scatter(x_ax(3)+noise_plot(n), score_SH(n), 5, col_(2,:), 'filled', 'MarkerFaceAlpha', 0.09); hold on;
        scatter(x_ax(6)+noise_plot(n), first_LH(n), 5, col_(2,:), 'filled', 'MarkerFaceAlpha', 0.09); hold on;
        lin2 = plot(x_ax([3 6])+noise_plot(n),[score_SH(n) first_LH(n)]); hold on;
        lin2.Color = [col_(2,:) 0.08]; % transparency
    end
